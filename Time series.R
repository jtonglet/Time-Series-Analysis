...
#Load Dataset
app <- read_delim("app.csv", ";", escape_double = FALSE, trim_ws = TRUE)
View(app)
attach(app)
dim(app)
...
#We estimate different models based on those specification

#SARIMA(1,0,1)(1,0,1)
model1 = arima(snew_users_ts, order = c(1,0,1), seasonal = c(1,0,1))
model1
# sar1 is not significant. We continue with more simple models

#SARIMA(1,0,1)(0,0,1)
model2 = arima(snew_users_ts, order = c(1,0,1), seasonal = c(0,0,1))
model2
#All coefficients are significant
 
#SARIMA(1,0,1)(1,0,0)
model3 = arima(snew_users_ts, order = c(1,0,1), seasonal = c(1,0,0))
model3
#All coefficients are significant

#We keep model 2 and 3 for the validation step
...
plot.ts(res2)
Box.test(res2, lag = 15, type = "Ljung-Box") #Residuals are white noise
#Model2 is validated
...
plot.ts(res3)
Box.test(res3, lag = 15, type = "Ljung-Box") #Residuals are white noise
#Model3 is validated
...
#Forecasts are made for model 2 and model 3

#Forecasts with model 2
forecast_m2 <- predict(model2, n.ahead = 24) 

expected_m2 <- forecast_m2$pred 
#Forecasted values are computed in differences. 
#We sum each forecasted value with the value on the same hour the day before

for(i in 1:length(expected_m2)){
  expected_m2[i] <- expected_m2[i] + new_users_ts[i+169-24]
}

#95% prediction intervals
lower_m2 <- expected_m2 - qnorm(0.975)*forecast_m2$se
upper_m2 <- expected_m2 + qnorm(0.975)*forecast_m2$se

plot.ts(new_users_ts,xlim= c(22,32),ylim = c(0,25))
lines(expected_m2, col ="red")
lines(upper_m2, col = "blue")
lines(lower_m2, col = "blue")


#Forecast with model 3
forecast_m3 <- predict(model3, n.ahead = 24) 

expected_m3 <- forecast_m3$pred 

for(i in 1:length(expected_m3)){
  expected_m3[i] <- expected_m3[i] + new_users_ts[i+169-24]
}

#95% prediction intervals
lower_m3 <- expected_m3 - qnorm(0.975)*forecast_m3$se
upper_m3 <- expected_m3 + qnorm(0.975)*forecast_m3$se

plot.ts(new_users_ts,xlim= c(22,32),ylim = c(0,25))
lines(expected_m3, col ="red")
lines(upper_m3, col = "blue")
lines(lower_m3, col = "blue")


#Expanding window approach
y <- snew_users_ts
S = round(0.75*length(y)) #109
h = 1
#Model 2
error2.h <- c()
for(i in S:(length(y)-h))
{
  mymodel.sub <- arima(y[1:i], order = c(1,0,1), seasonal = c(0,0,1))
  predict.h <- predict(mymodel.sub, n.ahead = h)$pred[h]
  error2.h <- c(error2.h, y[i+1],predict.h)
} 
#Model 3
error3.h <- c()
for(i in S:(length(y)-h))
{
  mymodel.sub <- arima(y[1:i], order = c(1,0,1), seasonal = c(1,0,0))
  predict.h <- predict(mymodel.sub, n.ahead = h)$pred[h]
  error3.h <- c(error3.h, y[i+1],predict.h)
}
...
active_users_ts <- ts(users, frequency = 24, start = c(22,9))
plot.ts(active_users_ts)

#Plot a multiple graph of new users and active users per hour
ts.plot(new_users_ts, active_users_ts, col = c("blue","orange"))


#Plot the correlogram of the TS
ggAcf(active_users_ts) #The TS seems not stationary. It shows a seasonal pattern.

#Perform the unit root test for stationarity
max.lag <- round(sqrt(length(new_users_ts))) #13
CADFtest(active_users_ts, type = "drift", criterion = "BIC", max.lag.y = max.lag)
#Strong evidence that the TS is not stationary

#Perform the Ljung-Box test for white noise
Box.test(active_users_ts,  lag = 15, type = "Ljung-Box")
...
#Perform the unit root test for stationarity
max.lag <- round(sqrt(length(dsactive_users_ts))) #12
CADFtest(dsactive_users_ts, type = "drift", criterion = "BIC", max.lag.y = max.lag)
#Strong evidence that the TS is now stationary

#Perform the Ljung-Box test for white noise
Box.test(dsactive_users_ts, lag = 15, type = "Ljung-Box")
#The TS is not white noise



#Model : Autoregressive DLM of order 2
lag <- 2
...
ggAcf(res_adlm)
plot.ts(res_adlm)
Box.test(res_adlm, lag = 15, type = "Ljung-Box")
#Model is validated

#test for Granger causality
fit_adlm_small <- lm(sY.0 ~sY.1+sY.2)
anova(fit_adlm, fit_adlm_small) 
